...
public class EmailCodeService {
    ...
    public EmailCodeService(EmailCodeRepository emailCodeRepository, UserRepository userRepository, EmailProducer emailProducer, BCryptPasswordEncoder passwordEncoder) {
    ...
    public void createPasswordResetEmail(String email) {
        Optional<User> userOpt = userRepository.findByEmail(email);
        if (userOpt.isEmpty()) {
            throw new IllegalArgumentException("User not found for email: " + email);
        }
        ...
        EmailCode emailCode = new EmailCode(user, code, EmailType.EMAIL_FORGOT_PASSWORD.name(), expiresAt);
        emailCodeRepository.save(emailCode);

        emailProducer.sendEmailTask(new EmailNotification(email, user.getName(), code, EmailType.EMAIL_FORGOT_PASSWORD));
    }


    public boolean verifyPasswordResetEmailCode(String email, int code, String password) {
        Optional<EmailCode> opt = emailCodeRepository.findByUserEmailAndCodeAndExpiresAtGreaterThan(email, code, LocalDateTime.now());
        if (opt.isEmpty()) {
            return false;
        }
